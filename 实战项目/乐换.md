...
​	当一个主系统有多个子系统的时候，可以通过消息中间件来实现解耦合。![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/1208468-20190126142747039-343663977.png)

#### **2）异步调用**

​	假设你有一个系统调用链路，是系统A调用系统B，一般耗时20ms；系统B调用系统C，一般耗时200ms；系统C调用系统D，一般耗时2s，如下图所示。

![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/1208468-20190126142757375-1232720078.png)

​	因为系统D耗时比较久，所以可以将系统D抽离出来，实现异步调用，即先不用急着启动系统D，等到上面的完成之后再启动也可以，系统D回去消费前面几个系统发过来的数据。

![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/1208468-20190126142809076-677253897.png)
...
![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/1208468-20190126143304043-1472821196.png)
...
即一个生产者和一个消费者一一对应，一个队列代表一个消息；![image-20190328095525250](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190328095525250.png)
...
![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/879896-20160604194640227-215496499.gif)
...
通过ActiveMQ管理界面查询：![image-20190328100634208](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190328100634208.png)
...
![image-20190328095533792](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190328095533792.png)
...
![img](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/879896-20160604232610055-1944763982.gif)
...
![image-20190328101805491](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190328101805491.png)
...
![image-20190330143401067](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190330143401067.png)

​	应用系统相当于web模块，访问应用系统1如果没登录会访问认证系统，认证系统连接到数据库，带着ticket票据做认证。

​	相比于单系统登录，sso需要一个独立的认证中心，**只有认证中心能接受用户的用户名密码等安全信息，其他系统不提供登录入口，只接受认证中心的间接授权。**间接授权通过令牌实现，sso认证中心验证用户的用户名密码没问题，创建授权令牌，在接下来的跳转过程中，授权令牌作为参数发送给各个子系统，子系统拿到令牌，即得到了授权，可以借此创建局部会话，局部会话登录方式与单系统的登录方式相同。这个过程，也就是单点登录的原理，用下图说明：

![image-20190525170338327](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190525170338327.png)

### 4.1 自己实现

#### 1.单点登录

![单点注销10](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/单点注销10.jpg)

> 图中最右边两个表是自定义的两个实体类对象
...
>
> 我们修改host的目的主要是来实现跨域的单点登录.
>
> 2.将资料中的client.zip和ssoServer.zip解压并导入到工具中.配置ssoServer的端口为:8443,配置client的端口为:8088

##### 客户端操作：

> 1.拦截客户端的请求判断是否有局部的session
>
> ​	通过Spring的拦截器或者Tomcat的Filter，注意要在web.xml中配置filter，然后创建一个过滤类，在过滤类中先对session进行判断
...
>             //表示没有全局会话
> //跳转到统一认证中心的登陆页面,将redirectUrl放入model里面，后面在jsp中通过${redirectUrl}获取
...
>             model.addAttribute("token",token);	// 把参数放入model里面
>             return "redirect:"+redirectUrl;
>         }
...
![image-20190526001233006](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190526001233006.png)
...
>         String token = (String) session.getAttribute("token");// 全局会话参数
>         //删除t_token表(数据库)中的数据
...
![image-20190524215350892](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190524215350892.png)
...
![image-20190524215753430](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190524215753430.png)
...
![image-20190331100325081](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331100325081.png)
...
//        创建订单明细商品，存入相应的信息
        ...
        orderItem.setPicPath(item.getImage());      // 缩略图路径
...
![image-20190331143837575](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331143837575.png)

预请求和预响应，先询问是否为跨域请求

Preflight Request：

![image-20190331143954341](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331143954341.png)

然后服务器端给我们返回一个Preflight Response

![image-20190331144017225](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331144017225.png)
...
![image-20190331153421732](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331153421732.png)

**order_id不用自增,自增可能会导致id重复，所以用分布式ID生成器**

![image-20190331153556274](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331153556274.png)
...
![image-20190331154941809](https://learningpics.oss-cn-shenzhen.aliyuncs.com/images/image-20190331154941809.png)
...
## 11.web.xml中\<context-param>和\<init-param>的区别
